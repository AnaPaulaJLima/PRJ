<?php
  ...
  function gerarCodigoUnicoV() {
    ...
    return "video_" . $codigo_data . "_" . $resultado;
  }

  //pega apenas a extensão do nome da imagem
  function getExtensaoV($nome) {
    //funcao pega caracteres a partir do caracterer selecionado
    return strrchr($nome,".");
  }

  function  publicarArquivoVideo( $video ) {
    $arquivo_tmp = $video["tmp_name"];
    $nome_original = basename( $video["name"] );
    $nome_novo = gerarCodigoUnicoV() . getExtensaoV($nome_original);
    $nome_completo = "../../../video/" . $nome_novo;
   ...
   function mostrarAvisoPublicacaoV( $numero ) {
